...
function buildItems(pool, seg) {
  ...
  id: usedImages[i].id ?? null,
  trailer: usedImages[i].trailer ?? null,
}));
}
...
export default function DomeGallery({
  ...
  const pool = useMemo(() => {
    if (Array.isArray(itemsProp) && itemsProp.length) {
      // itemsProp có thể là [{src, alt}] -> dùng luôn
      return itemsProp;
    }
    if (Array.isArray(imagesProp) && imagesProp.length) {
      // imagesProp có thể là string[] hoặc object[]
      return imagesProp
        .map((it) =>
          typeof it === "string"
            ? { src: it, alt: "" }
  ...
  }, [itemsProp, imagesProp]);

  const tiles = useMemo(() => buildItems(pool, segments), [pool, segments]);
  ...
  const openItemFromElement = (el) => {
    ...
    const meta = {
      ...
      trailer: parent.dataset.trailer || null,
    title: rawAlt || null,
       alt: rawAlt || null,
  src: rawSrc || null, 
    };
...
`;
  ...
  `;
    ...
    const trailerBtn = mkBtn("Xem trailer", "#0ea5e9");
    trailerBtn.onclick = (e) => {
      e.stopPropagation();
      if (typeof onOpenTrailer === "function") onOpenTrailer(meta);
      else if (meta.trailer) window.open(meta.trailer, "_blank");
    };

    const detailBtn = mkBtn("Xem chi tiết", "#22c55e");
    detailBtn.onclick = (e) => {
      e.stopPropagation();
      if (typeof onOpenDetail === "function") onOpenDetail(meta);
    };
  ...
  /* Khi phóng to ảnh: bật scrim & cho phép overlay nhận click */
  .sphere-root[data-enlarging="true"] .scrim {
    opacity: 1 !important;
    pointer-events: all !important;
  }
  .sphere-root[data-enlarging="true"] .viewer {
    pointer-events: auto;
  }
  .enlarge, .enlarge * {
    pointer-events: auto;
  ...
  /* NOTE: nếu muốn khoá scroll body thì dùng comment đúng chuẩn:
  body.dg-scroll-lock {
